...
try:
    with open('monkey.names','r') as cls:
    ...
    sys.exit(1)
COLORS = random.sample(set(MAIN_COLORS), len(MAIN_COLORS))#, len(classes))
    ...
    def loadDir(self, dbg = False):
        if not dbg:
            try:
                ...
                return
        if not os.path.isdir('./monkey_data_new/%s' % self.category):
           tkMessageBox.showerror("Error!", message = "The specified dir doesn't exist!")
           return
        # get image list
        self.imageDir = os.path.join(r'./monkey_data_new', '%s' %(self.category))
        ...
        self.total = len(self.imageList)
         ...
         # set up output dir
        if not os.path.exists('./monkey_data_new'):
            os.mkdir('./monkey_data_new')
        self.outDir = os.path.join(r'./monkey_data_new', '%s' %(self.category))
