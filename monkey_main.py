...
try:
    with open('data/monkey.names','r') as cls:
    ...
    def loadDir(self, dbg = False):
        if not dbg:
            try:
                ...
                return
        if not os.path.isdir('./monkey_data/%s' % self.category):
           tkMessageBox.showerror("Error!", message = "The specified dir doesn't exist!")
           return
        # get image list
        self.imageDir = os.path.join(r'./monkey_data', '%s' %(self.category))
        ...
        self.total = len(self.imageList)
         ...
         # set up output dir
        if not os.path.exists('./monkey_data'):
            os.mkdir('./monkey_data')
        self.outDir = os.path.join(r'./monkey_data', '%s' %(self.category))
